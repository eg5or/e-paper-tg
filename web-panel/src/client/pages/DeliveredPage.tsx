...
const REFRESH_STORAGE_KEY = "paper.delivered.refreshSeconds";
const DEFAULT_REFRESH_SECONDS = 20;
const refreshOptions = [3, 5, 10, 15, 20, 30, 45, 60];

function formatUnixTime(unixTs: number | null | undefined): string {
  if (!unixTs) return "-";
...
export function DeliveredPage() {
  ...
  const deliveredQuery = useQuery({
    queryKey: ["delivered"],
    queryFn: api.delivered,
    refetchInterval: refreshMs
  });

  const rows = deliveredQuery.data || [];
    ...
    <Stack spacing={2}>
      <PageCard>
        <Stack direction={{ xs: "column", md: "row" }} spacing={2} alignItems={{ xs: "stretch", md: "center" }}>
          <Box sx={{ flexGrow: 1 }}>
            <Typography variant="h6">Отправленные команды</Typography>
            <Typography variant="body2" color="text.secondary">
              Показаны команды со статусом delivered.
            </Typography>
          </Box>
          <FormControl size="small" sx={{ minWidth: 220 }}>
            <InputLabel id="delivered-refresh-label">Обновление</InputLabel>
            <Select
              labelId="delivered-refresh-label"
              label="Обновление"
      ...
      {deliveredQuery.isError ? (
        <Alert severity="error">
          {deliveredQuery.error instanceof Error ? deliveredQuery.error.message : "Ошибка загрузки"}
        </Alert>
      ) : null}
          ...
          <TableBody>
            {rows.map((row) => (
              <TableRow key={row.id}>
                ...
                <TableCell>{formatUnixTime(row.deliveredAt)}</TableCell>
